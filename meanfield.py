...
from  utils import unpackEdgeFeature,packEdgeFeature
import time
...
MAXN=15 # At most 10 people

H=[] # list of lists # N*(N-1)/2
HP=[] # N*(N-1)
...
def generate_HSet():
    ...
    for n in range(2,MAXN+1):
        ...
        tmp_perm=[[*p,i] for i,p in enumerate(permutations(range(n),2))]
        H.append(tmp_comb)
        HP.append(tmp_perm)
        d={}
...
def meanfield(actn_score,intrct_score,lambda_h,lamda_g,conflict):
    ...
    for t in range(6):
        ...
        cur_actn_score, cur_intrct_score = one_step(Q_y,Q_z,lambda_h,lamda_g,conflict)
...
def one_step(Qy, Qz, lambda_h, lamda_g,conflict):
    # Update Q(y)
    global mask_list,Z_map,Z_map_r
    device=Qy.device
    mask=torch.Tensor(conflict).to(device).long()
    ...
    # h=torch.Tensor(H[N]).long()#(y,y,z) from N*(N-1)/2
    # cc=m[h[:,0],h[:,1],:,:]
    # poly_h[Z_map_N[h[:,2]],1]=torch.sum(cc,dim=[1,2]).unsqueeze(-1)*lambda_h
    hp=torch.Tensor(HP[N]).long().to(Qz.device)#(y,y,z) from N*(N-1)
    cc=m[hp[:,0],hp[:,1],:,:]
    poly_h[:,1]=torch.sum(cc, dim=[1, 2])* lambda_h
...
if __name__=='__main__':
    N=4
    setSeed(87)
    score_y=torch.randn((N,10)).cuda() # assume this is energy
    score_z=torch.zeros(N,N,2).cuda()
    for i in range(N):
        for j in range(N):
            if i < j:
                score_z[i][j] = torch.randn((1,2))
                score_z[j][i] = score_z[i][j]
    score_z = packEdgeFeature(score_z, N)

    #
    tstart=time.time()
    y_score,z_score = meanfield(score_y,score_z,0.5,1.9)
    tend = time.time()
    print(score_y)
    print('y prob:')
