...
class GCN_Module(nn.Module):
    ...
    def forward(self, graph_boxes_features, boxes_in_flat):
        ...
        position_mask = (graph_boxes_distances > (pos_threshold * OW))# horizontal orient
        ...
        return graph_boxes_features, relation_graph,position_mask
...
class LAGNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        for p in self.backbone.parameters():
            p.requires_grad = cfg.train_backbone
    ...
    def forward(self, batch_data):
        ...
        boxes_idx = [i * torch.ones(MAX_N, dtype=torch.int) for i in
                     range(B * T)]
        ...
        for bt in range(B * T):
            ...
            actn_score,A,position_mask=self.gcn(boxes_states_flat,boxes_in[bt,:N])
            ...
            interaction_score=torch.stack((1-G,G),dim=1)

            Q_y,Q_z=self.meanfield(actn_score, interaction_score,
                              self.lambda_h,self.lambda_g,self.cfg.conflict)
            Q_z = unpackEdgeFeature(Q_z,N)
            cord=position_mask.nonzero()
            Q_z[cord[:,0],cord[:,1],:] = torch.Tensor([1,0]).to(Q_z.device)
            Q_z=packEdgeFeature(Q_z,N)
        ...
        return actions_scores, interaction_scores, A_list
